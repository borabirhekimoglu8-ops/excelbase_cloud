...
async def passport_ocr_recognize(
    ...
    if token and (len(token) > 64 or any(ch not in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-" for ch in token)):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="page_id geçersiz.")
    ...
    try:
        ...
        if reason == "too_large":
            raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Görüntü çok büyük.") from None
        if reason == "unsupported":
            raise HTTPException(status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE, detail="Yalnız JPEG, PNG veya HEIC kabul edilir.") from None
        if reason == "too_many_pixels":
            raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Görüntü çözünürlüğü çok yüksek.") from None
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Görüntü okunamadı.") from None
    except TimeoutError:
        logger.info("passport-ocr recognize request_id=%s status=%d lines=%d duration_ms=%d", request_id, 429, 0, 0)
        return JSONResponse(status_code=429, content={"detail": "OCR meşgul, tekrar deneyin.", "state": "busy"}, headers={"Retry-After": "2"})
    except EngineUnavailable as exc:
        logger.info("passport-ocr recognize request_id=%s status=%d lines=%d duration_ms=%d", request_id, 503, 0, 0)
        return _passport_ocr_http_error(exc.state, exc.detail, status.HTTP_503_SERVICE_UNAVAILABLE)
    logger.info(
        "passport-ocr recognize request_id=%s status=%d lines=%d duration_ms=%d",
        request_id,
        200,
